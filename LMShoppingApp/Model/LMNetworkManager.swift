...
class LMNetworkManager {
    ...
    class func fetchProductList(completion: @escaping (NSDictionary?, Error?) -> Void) {
        ...
        let dataTask = session.dataTask(with: url) { (data, response, error) in
            ...
            guard let unwrappedData = data else { print("Error getting data"); completion(nil,error); return }
